...
export class SikayetTurForm extends React.Component {
  state = {
    form: {
      id: null,
      ad: null,
    ...
    validation: {
      ...
      'mudurluk.id': defaultValidationState(),
  ...
  mapInitialFormState = () => {
    ...
    return this.setState(
      produce(draft => {
        ...
        _.set(redraft.form, 'mudurluk.id', _.get(data, 'mudurluk.id', null));
        ...
        names.forEach(name => {
          ...
          if (name === 'mudurluk.id') {
            if (form.mudurluk === null) {
              _.set(redraft.validation, name, nonValidState('Lütfen müdürlük bilgisi giriniz.'));
            } else {
              _.set(redraft.validation, name, validState());
            }
          }
  ...
  render() {
    ...
    const { mudurlukler } = this.props.resources;
      ...
      <Form>
        ...
        <Form.Group widths="equal">
          <Form.Field>
            <Form.Dropdown
              label="Müdürlük"
              placeholder="Müdürlük giriniz"
              name="mudurluk.id"
              value={_.get(form, 'mudurluk.id')}
              options={mudurlukler}
              onChange={(e, data) => {
...
SikayetTurForm.propTypes = {
  data: PropTypes.object,
  onChange: PropTypes.func,
  resources: PropTypes.object,
};

SikayetTurForm.defaultProps = {
  resources: {
    mudurlukler: [],
  },
};

export default SikayetTurForm;
